...
def add_medcat_annotation_to_obs(
    ...
    if name not in df["pretty_name"].values:
        ...
        if new_name:
            msg = f"Did not find '{name}' in MedCAT's extracted entities. Do you mean {new_name}?"
        else:
            msg = f"Did not find '{name}' in MedCAT's extracted entities."
        raise EntitiyNotFoundError(msg)

    adata.obs[added_colname] = (
