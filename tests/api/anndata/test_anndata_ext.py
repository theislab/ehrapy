...
from ehrapy.api.anndata_ext import df_to_anndata, anndata_to_df, ObsEmptyError
...
class TestAnndataExt:
    ...
    def test_df_to_anndata_cols_obs_only(self):
        ...
        assert_frame_equal(adata.obs, DataFrame({"col1": col1_val, "col2": col2_val}, index=[str(idx) for idx in range(100)]))
    ...
    def test_anndata_to_df_simple(self):
        ...
        adata = AnnData(X=adata_x, obs=DataFrame(index=[idx for idx in range(100)]), var=DataFrame(index=["col"+str(idx) for idx in range(1, 4)]), dtype="object")
    ...
    def test_anndata_to_df_throws_error_with_empty_obs(self):
        col1_val = ["patient" + str(idx) for idx in range(100)]
        adata_x = np.array([col1_val], dtype="object").transpose()
        adata = AnnData(X=adata_x, obs=DataFrame(index=[idx for idx in range(100)]), var=DataFrame(index=["col1"]), dtype="object")
