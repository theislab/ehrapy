...
class TestRead:
    ...
    def test_read_tsv(self):
        adata = DataReader.read(dataset_path=f"{_TEST_PATH}/dataset2.tsv", delimiter="\t")
        matrix = np.array(
            [
                [12, 54, 185.34, False],
                [13, 25, 175.39, True],
    ...
    def test_set_given_str_index(self):
        adata = DataReader.read(dataset_path=f"{_TEST_PATH}/dataset1.csv", index_column="los_days")
    ...
    def test_set_given_int_index(self):
        adata = DataReader.read(dataset_path=f"{_TEST_PATH}/dataset1.csv", index_column=1)
        assert adata.X.shape == (5, 3)
        assert adata.obs_names.name == "los_days"
        assert list(adata.obs.index.values) == ["14", "7", "10", "11", "3"]
    ...
    def test_move_single_column_misspelled(self):
        with pytest.raises(ColumnNotFoundError):
            adata = DataReader.read(  # noqa: F841
                dataset_path=f"{_TEST_PATH}/dataset1.csv", columns_obs_only=["b11_values"]
            )

    def test_move_single_column_to_obs(self):
        adata = DataReader.read(dataset_path=f"{_TEST_PATH}/dataset1.csv", columns_obs_only=["b12_values"])
        assert adata.X.shape == (5, 3)
    ...
    def test_move_multiple_columns_to_obs(self):
        adata = DataReader.read(dataset_path=f"{_TEST_PATH}/dataset1.csv", columns_obs_only=["b12_values", "survival"])
