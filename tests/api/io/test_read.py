...
class TestRead:
    def test_read_csv(self):
        ann_data = DataReader.read(file_path=f"{_TEST_PATH}/dataset1.csv")
        matrix = np.array(
    ...
    def test_read_tsv(self):
        ann_data = DataReader.read(file_path=f"{_TEST_PATH}/dataset2.tsv", delimiter="\t")
        matrix = np.array(
            [
                [12, 54, 185.34, False],
                [13, 25, 175.39, True],
    ...
    def test_read_csv_without_index_column(self):
        ann_data = DataReader.read(file_path=f"{_TEST_PATH}/dataset3.csv")
        matrix = np.array(
    ...
    def test_set_given_str_index(self):
        ann_data = DataReader.read(file_path=f"{_TEST_PATH}/dataset1.csv", index_column="los_days")
    ...
    def test_set_given_int_index(self):
        ann_data = DataReader.read(file_path=f"{_TEST_PATH}/dataset1.csv", index_column=1)
        assert ann_data.X.shape == (5, 3)
        assert ann_data.obs_names.name == "los_days"
        assert list(ann_data.obs.index.values) == ["14", "7", "10", "11", "3"]
    ...
    def test_move_single_column_misspelled(self):
        with pytest.raises(ColumnNotFoundError):
            ann_data = DataReader.read(  # noqa: F841
                file_path=f"{_TEST_PATH}/dataset1.csv", columns_obs_only=["b11_values"]
            )

    def test_move_single_column_to_obs(self):
        ann_data = DataReader.read(file_path=f"{_TEST_PATH}/dataset1.csv", columns_obs_only=["b12_values"])
        assert ann_data.X.shape == (5, 3)
    ...
    def test_move_multiple_columns_to_obs(self):
        ann_data = DataReader.read(file_path=f"{_TEST_PATH}/dataset1.csv", columns_obs_only=["b12_values", "survival"])
