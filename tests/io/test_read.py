...
class TestRead:
    ...
    def test_read_tsv(self):
        adata = read_csv(dataset_path=f"{_TEST_PATH}/dataset2.tsv", sep="\t")
        matrix = np.array(
            [
                [12, 54, 185.34, False],
                [13, 25, 175.39, True],
    ...
    def test_read_multiple_csv(self):
        ...
        assert set(adatas["dataset8"].var_names) == {"indexcol", "intcol", "strcol", "boolcol", "binary_col"}
        assert set(adatas["dataset9"].var_names) == {"col" + str(i) for i in range(1, 4)}

    def test_read_multiple_csv_with_obs_only(self):
        adatas = read_csv(
            dataset_path=f"{_TEST_PATH_MULTIPLE}", columns_obs_only={"dataset8": ["strcol"], "dataset9": ["col1"]}
        )
        adata_ids = set(adatas.keys())
        assert all(adata_id in adata_ids for adata_id in {"dataset8", "dataset9"})
        assert set(adatas["dataset8"].var_names) == {"indexcol", "intcol", "boolcol", "binary_col"}
        assert set(adatas["dataset9"].var_names) == {"col" + str(i) for i in range(2, 4)}
        assert all(obs_name in set(adatas["dataset8"].obs.columns) for obs_name in {"datetime", "strcol"})
        assert "col1" in set(adatas["dataset9"].obs.columns)
    ...
    def test_read_csv_without_index_column(self):
        adata = read_csv(dataset_path=f"{_TEST_PATH}/dataset3.csv")
        matrix = np.array(
    ...
    def test_read_csv_with_bools_obs_only(self):
        adata = read_csv(dataset_path=f"{_TEST_PATH}/dataset1.csv", columns_obs_only=["survival", "b12_values"])
    ...
    def test_read_csv_with_bools_and_cats_obs_only(self):
        adata = read_csv(dataset_path=f"{_TEST_PATH}/dataset6.csv", columns_obs_only=["b12_values", "name", "survival"])
    ...
    def test_set_given_str_index(self):
        adata = read_csv(dataset_path=f"{_TEST_PATH}/dataset1.csv", index_column="los_days")
    ...
    def test_set_given_int_index(self):
        adata = read_csv(dataset_path=f"{_TEST_PATH}/dataset1.csv", index_column=1)
        assert adata.X.shape == (5, 3)
        assert adata.obs_names.name == "los_days"
        assert list(adata.obs.index.values) == ["14", "7", "10", "11", "3"]
    ...
    def test_move_single_column_misspelled(self):
        with pytest.raises(ColumnNotFoundError):
            adata = read_csv(dataset_path=f"{_TEST_PATH}/dataset1.csv", columns_obs_only=["b11_values"])  # noqa: F841

    def test_move_single_column_to_obs(self):
        adata = read_csv(dataset_path=f"{_TEST_PATH}/dataset1.csv", columns_obs_only=["b12_values"])
        assert adata.X.shape == (5, 3)
    ...
    def test_move_multiple_columns_to_obs(self):
        adata = read_csv(dataset_path=f"{_TEST_PATH}/dataset1.csv", columns_obs_only=["b12_values", "survival"])
        assert adata.X.shape == (5, 2)
